...
const rssFeedUrl = "https://feeds.acast.com/public/shows/6728ae32dc854c9577f0ce16"; // Utilise ton flux réel ici
...
async function loadPodcast() {
    try {
        ...
        for (const item of items) {
            ...
            if (audioUrl) {
                await prefetchAudio(audioUrl); // Précharge les 3 Mo
...
function displayEpisode(title, audioUrl, description, imageUrl) {
    ...
    audioElement.src = audioUrl; // L'URL complète pour la lecture continue
...
async function prefetchAudio(url, maxBytes = 3 * 1024 * 1024) {
    try {
        ...
        console.log(`Préchargement des 3 Mo de ${url} réussi.`);
    } catch (error) {
        console.warn(`Erreur de préchargement pour ${url}: ${error.message}`);
    }
}
